...
// --- Reducer case handlers (extracted to keep `reducer` short & focused) ----
...
function useToast() {
  ...
  React.useEffect(() => {
    ...
    // `setState` is stable from useState; `listeners` is a module-level
    // singleton. Subscribing on mount / unsubscribing on unmount is correct,
    // so the dependency array is intentionally empty.
  }, [])
